...
Stages 1–4 are complete. The agent loop writes every API call, tool call, and message as an immutable event row; conversation history is projected from the events table at turn start; a post-turn meta-call sets the projection bound for the next turn as a `context_decision` event. Stage 5 (Telegram bot integration) is next. Monitoring and hardening are deferred. See `PROJECT_PLAN.md` for the full stage breakdown.
