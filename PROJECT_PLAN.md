...
- **`messages` is seeded from a projection of events.** At the start of each turn, the `messages` array is built by querying the events table (chronological projection of the current conversation). Within the turn, new messages (tool calls, tool results, assistant responses) are appended to that in-memory list as they arrive — events are written to the DB in parallel for durability, but the active turn runs against the in-memory accumulator, not re-derived from the DB on each API call. When the turn ends the in-memory list is discarded; the next turn re-projects from the DB. The projection policy (simple chronological → pruned → summarized) can evolve without changing the underlying ledger.
...
**3c. Messages-as-projection.** At turn start, project the events table into the `messages` array that seeds the Claude conversation for this turn. The projection runs once per turn (not once per API call); within the turn, the agent loop appends to an in-memory accumulator as tool calls and results arrive. Start with the simplest possible projection: chronological, current conversation, all message-like event types. This is the point where Stage 2b's former "context management" goal lives — it becomes "improve the projection policy" rather than a separate subsystem.
